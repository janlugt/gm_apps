...
public class hop_dist implements Tool {
    // Class logger
    private static final Logger LOG = Logger.getLogger(hop_dist.class);
    ...
    public final int run(final String[] args) throws Exception {
        ...
        job.getConfiguration().setVertexClass(hop_distVertex.class);
        job.getConfiguration().setMasterComputeClass(hop_distVertex.Master.class);
        job.getConfiguration().setWorkerContextClass(hop_distVertex.Context.class);
        job.getConfiguration().setVertexInputFormatClass(hop_distVertexInputFormat.class);
        GiraphFileInputFormat.addVertexInputPath(job.getInternalJob(), new Path(cmd.getOptionValue('i')));
        if (cmd.hasOption('o')) {
            job.getConfiguration().setVertexOutputFormatClass(hop_distVertexOutputFormat.class);
        ...
        job.getConfiguration().setLong("root", Long.parseLong(cmd.getOptionValue("root")));
    ...
    public static void main(final String[] args) throws Exception {
        System.exit(ToolRunner.run(new hop_dist(), args));
    }
}
