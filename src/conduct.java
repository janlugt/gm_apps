...
public class conduct implements Tool {
    // Class logger
    private static final Logger LOG = Logger.getLogger(conduct.class);
    ...
    public final int run(final String[] args) throws Exception {
        ...
        options.addOption("_num", "num", true, "num");
        ...
        if (!cmd.hasOption("num")) {
            LOG.info("Need to set procedure argument (--num)");
            return -1;
        }
        ...
        job.getConfiguration().setVertexClass(conductVertex.class);
        job.getConfiguration().setMasterComputeClass(conductVertex.Master.class);
        job.getConfiguration().setWorkerContextClass(conductVertex.Context.class);
        job.getConfiguration().setVertexInputFormatClass(conductVertexInputFormat.class);
        GiraphFileInputFormat.addVertexInputPath(job.getInternalJob(), new Path(cmd.getOptionValue('i')));
        if (cmd.hasOption('o')) {
            job.getConfiguration().setVertexOutputFormatClass(conductVertexOutputFormat.class);
        ...
        job.getConfiguration().setInt("num", Integer.parseInt(cmd.getOptionValue("num")));
    ...
    public static void main(final String[] args) throws Exception {
        System.exit(ToolRunner.run(new conduct(), args));
    }
}
